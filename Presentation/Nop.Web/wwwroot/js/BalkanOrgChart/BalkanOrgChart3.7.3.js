...
var OrgChart = function(b, a) {
    ...
    if (a) {
        for (var e in this.config) {
            if (typeof a[e] === "object" && !Array.isArray(a[e])) {
                this.config[e] = BALKANGraph._9(this.config[e], a[e])
            } else {
    ...
    if (!BALKANGraph._aZ(this.config)) {
        return
    }
    this.server = null;
    this._aW = {};
...
OrgChart.prototype._m = function(s, a, c, i) {
    ...
    for (var o in t.visibleNodes) {
        ...
        k += this.ui.node(g, this._Q(o), t.animations, this.config, undefined, undefined, undefined, a);
    ...
    if (a === BALKANGraph.action.exporting) {
        ...
        var n = this.ui.svg(z, m, [(f.minX - this.config.padding), (f.minY - this.config.padding), z, m], this.config, k);
        i(n);
        return
    }
    this.response = t;
...
OrgChart.prototype._au = function() {
...
OrgChart.prototype.linkNode = function(b, j) {
    if (this._r(b, j)) {
        ...
        if (c.isChildOfGroup) {
            this.removeNodeTag(c.id, c._U);
            var d = this.getBGNode(c._U);
            if (d.bgnodes.length == 2) {
                for (var e = 0; e < d.bgnodes.length; e++) {
                    var a = d.bgnodes[e];
                    if (c.id != a.id) {
                        this.removeNodeTag(a.id, a._U)
                    }
                }
            }
        }
...
OrgChart.prototype.removeNode = function(b, a) {
    ...
    this._m(false, BALKANGraph.action.update, null, function() {
        BALKANGraph._aw(c.getSvg(), c.getViewBox(), c.response.boundary)
    })
};
...
OrgChart.prototype._aA = function(a, b, f) {
    var d = JSON.parse(JSON.stringify(this.config.tags));
    d[a][b] = f;
...
OrgChart.prototype._ag = function(d, b) {
...
OrgChart.prototype.group = function(h, l) {
    ...
    if (!g.isGroup && !k.isGroup && !g.isChildOfGroup && !k.isChildOfGroup) {
        ...
        if (!g.isGroup && !k.isGroup && g.isChildOfGroup && !k.isChildOfGroup) {
            ...
            this._ag(g._U, g.id);
            this.removeNodeTag(g.id, g._U, true);
            this.addNodeTag(g.id, c, true);
            this.addNodeTag(k.id, c, true);
            var f = this._Q(k.id).pid;
            if (g.id != f) {
            ...
            if (!g.isGroup && !k.isGroup && !g.isChildOfGroup && k.isChildOfGroup) {
                ...
                if (!g.isGroup && !k.isGroup && g.isChildOfGroup && k.isChildOfGroup) {
                    this._ag(g._U, g.id);
                    this.removeNodeTag(g.id, g._U, true);
                    this.addNodeTag(g.id, k._U, true)
                }
            }
        }
    }
...
OrgChart.prototype._az = function(b, a, c) {
    c[0] = a;
    c[1] = b;
    this.setViewBox(c);
    this.xScrollUI.setPosition();
...
OrgChart.prototype._aq = function(g, b) {
    var h = this;
    ...
    var c = function() {
        if (a.diffX = !0 || a.diffY != 0) {
            BALKANGraph._aw(g, h.getViewBox(), h.response.boundary, function() {
...
OrgChart.editUI.prototype._b = function(h, b) {
    ...
    for (var f = 0; f < d.length; f++) {
        ...
        if (BALKANGraph._1(this.obj.config, d[f])) {
...
OrgChart.editUI.prototype._y = function(l, e) {
    ...
    for (var j = 0; j < g.length; j++) {
        ...
        if (g[j] != "tags") {
            if (BALKANGraph._1(this.obj.config, g[j])) {
                if (o) {
...
OrgChart.editUI.prototype.hide = function(f) {
    ...
    if (f) {
        ...
        var g = this;
        if (this.node.isGroup) {
            this.obj._aA(this.node.id, "groupName", b.Name)
        } else {
...
OrgChart.ui = {
    ...
    defs: function() {
        ...
        for (var c in OrgChart.templates) {
            ...
            if (b.defs) {
                OrgChart.ui._n[c] = BALKANGraph._af();
    ...
    node: function(d, g, c, e, r, u, l, a) {
        var p = OrgChart.templates[d.templateName];
        var m = p.node.replaceAll("{w}", d.w).replaceAll("{h}", d.h);
        if (p.defs) {
            m = m.replace("{randId}", OrgChart.ui._n[d.templateName])
        }
        if (l == undefined) {
            l = e.nodeBinding
        }
        if (d.isGroup) {
            var j = e.tags[d.id].groupName;
            if (j) {
                var i = p.groupName.replace("{val}", j);
                i = i.replaceAll("{randId}", BALKANGraph._af()).replaceAll("{randId2}", BALKANGraph._af());
                m += i
            }
            m += i
        } else {
            for (var h in l) {
                var q = l[h];
                var o = g[q];
                if (o != undefined && o != null && p[h] != undefined) {
                    o = BALKANGraph._aS(o, p[h]);
                    var i;
                    if (!BALKANGraph._1(e, h) && o.encodeHTML) {
                        i = p[h].replace("{val}", o.encodeHTML())
                    } else {
                        i = p[h].replace("{val}", o)
                    }
                    i = i.replaceAll("{randId}", BALKANGraph._af()).replaceAll("{randId2}", BALKANGraph._af());
        ...
        if (v == undefined) {
            if (c[d.id] != undefined && c[d.id].from.transform) {
                v = c[d.id].from.transform[5]
            }
        }
        if (v == undefined) {
            v = d.y
        }
        var n = BALKANGraph.nodeOpenTag.replace("{id}", d.id).replace("{class}", "node " + d.tags.join(" ")).replace("{level}", d.level).replace("{x}", s).replace("{y}", v);
        if (c[d.id]) {
            var b = c[d.id];
            if (b.from.opacity != undefined) {
                n = n.replace("{opacity}", b.from.opacity)
            }
        } else {
            n = n.replace("{opacity}", 1)
        }
        if (!d.isGroup && e.nodeMenu != null && a !== BALKANGraph.action.exporting) {
            m += p.nodeMenuButton.replace("{id}", d.id)
        }
        var k = OrgChart.ui._ab(d, p, e, c, l, r, u, a);
        if (d.isGroup && a != BALKANGraph.action.exporting) {
            if (d.groupState == BALKANGraph.EXPAND) {
                m += BALKANGraph.MINIMIZE.replace("{x}", d.w - (45)).replace("{id}", d.id)
            } else {
                m += BALKANGraph.MAXIMIZE.replace("{x}", d.w - (45)).replace("{id}", d.id)
            }
        }
        m = n + m + k + BALKANGraph.grCloseTag;
        return m
    },
    _ab: function(c, m, e, b, l, n, o, a) {
        var g = "";
        if (c.isGroup && c.groupState == BALKANGraph.EXPAND) {
            ...
            g = BALKANGraph.groupNodesOpenTag.replace("{x}", m.groupPadding[3]).replace("{y}", m.groupPadding[0]) + g + BALKANGraph.grCloseTag
        }
        return g
    },
...
OrgChart.server.prototype.read = function(j, o, d, n, a, b) {
    ...
    var k = OrgChart.server._V(o, d, this.visibleNodes, this.config, e, f, g, h, this.viewBox, m, this.action, this.actionParams, i, c, this.oldNodes);
    if (a != BALKANGraph.action.exporting) {
        this.maxX = e;
        this.maxY = f;
        this.minX = g;
...
OrgChart.server.prototype._av = function() {
    ...
    for (var b = 0; b < k.length; b++) {
        OrgChart.server._p(k[b], 0, d, e, h, this.config);
        OrgChart.server._ah(k[b], 0, 0, 0, d, e, this.config)
    }
    ...
    for (var c in g) {
        var f = g[c];
        OrgChart.server._al(f, this.config);
        OrgChart.server._ai(f, a)
    }
    return {
        minX: a.minX,
        minY: a.minY,
...
OrgChart.server.prototype.search = function(n) {
    ...
    for (var e = 0; e < this.config.nodes.length; e++) {
        ...
        for (var k in h) {
            var l = h[k];
            if (l && l.toString().toLowerCase().indexOf(n) != -1 && !BALKANGraph._1(this.config, k)) {
                d.push({
                    id: h.id,
                    propertyName: k,
    ...
    for (var e = 0; e < d.length; e++) {
        ...
        for (var k in d[e].data) {
            ...
            if (k == BALKANGraph.TAGS) {
                ...
                if (k == BALKANGraph.ID) {
                    ...
                    if (BALKANGraph._1(this.config, k)) {
                        continue
                    } else {
                        if (f(k) && !BALKANGraph._1(this.config, k)) {
                            r = r + l + ", "
                        } else {
                            if (!BALKANGraph._1(this.config, k)) {
                                p = p + l + ", "
        ...
        var o = BALKANGraph._B(d[e].tags, this.config.tags, this.config.template);
        m.push({
            id: d[e].id,
...
OrgChart.server._h = function(e, a, b, w) {
    ...
    for (var p = 0; p < g.length; p++) {
        ...
        var k = BALKANGraph._X(z, e.tags);
        var A = BALKANGraph._B(z, e.tags, e.template);
    ...
    for (var p = 0; p < t.length; p++) {
        ...
        if (v) {
            s.state = v.state;
            OrgChart.server._ao(s, v.groupState, e.orientation)
        } else {
            s.state = BALKANGraph.EXPAND;
            OrgChart.server._ao(s, BALKANGraph.EXPAND, e.orientation)
        }
        OrgChart.server._L(s, u, a, e, b, l);
        OrgChart.server._I(s, u, a, e, b)
    }
...
OrgChart.server._L = function(j, k, a, e, b, g) {
...
OrgChart.server._I = function(e, f, a, c, b) {
    if (a == BALKANGraph.action.exporting && b.expandChildren == true && e) {
        OrgChart.server._ao(e, BALKANGraph.EXPAND, c.orientation);
        return
    } else {
        if (a == BALKANGraph.action.init && e) {
            if (e.isGroup) {
                var d = c.tags[e.id].groupState;
                OrgChart.server._ao(e, d, c.orientation)
            }
        }
    }
    ...
    OrgChart.server._ao(f[b.id], b.state, c.orientation)
};
OrgChart.server._an = function(e, f) {
    if (e.isGroup && e.groupState == BALKANGraph.EXPAND) {
        ...
        if (k.layout_type == 0) {
            ...
            for (var d = 0; d < e.bgnodes.length; d++) {
                var b = e.bgnodes[d];
                b.x = e._J._v / 2 - b.w / 2 + k.groupNodesSeparation + (e._J._v * a + k.groupNodesSeparation * a);
                b.y = e._J._f / 2 - b.h / 2 + k.groupNodesSeparation + (e._J._f * j + k.groupNodesSeparation * j);
                if (a >= e._J._g - 1) {
                    a = 0;
                    j++
                } else {
...
OrgChart.server._O = function(j, o, k) {
...
OrgChart.server._ao = function(a, c, b) {
    a.groupState = c;
    OrgChart.server._O(a, a.templateName, b)
};
OrgChart.server._A = function(b, a) {
    return b.children[a]
};
OrgChart.server._W = function(a) {
    return OrgChart.server._A(a, 0)
};
OrgChart.server._E = function(a) {
    return OrgChart.server._A(a, a.children.length - 1)
};
OrgChart.server._Y = function(a) {
    var b = a.parent;
    var c = null;
    while (b != null) {
        if (b.state == BALKANGraph.COLLAPSE) {
...
OrgChart.server._Z = function(a) {
    if (a.state == BALKANGraph.COLLAPSE) {
        return 0
    }
    return a.children.length
};
OrgChart.server._T = function(a) {
...
OrgChart.server._F = function(b, a) {
...
OrgChart.server._D = function(g, d, e) {
    if (d >= e) {
        return g
    }
    if (OrgChart.server._Z(g) == 0) {
        return null
    }
    var f = OrgChart.server._Z(g);
    for (var a = 0; a < f; a++) {
        var b = OrgChart.server._A(g, a);
        var c = OrgChart.server._D(b, d + 1, e);
        if (c != null) {
            return c
        }
    }
...
OrgChart.server._S = function(d, b) {
...
OrgChart.server._2 = function(e, b, f, a) {
    if (e.x == null || e.y == null) {
        return
    }
    if (OrgChart.server._Y(e)) {
        return false
    }
...
OrgChart.server._al = function(e, a) {
    if (e.assistants.length > 0) {
        switch (a.orientation) {
            ...
            case BALKANGraph.orientation.bottom:
            case BALKANGraph.orientation.bottom_left:
                var c = e.x + OrgChart.server._F(e, a) + a.siblingSeparation;
                for (var b = 0; b < e.assistants.length; b++) {
                    e.assistants[b].y = e.y + e.h / 2 - e.assistants[b].h / 2;
                    e.assistants[b].x = c;
                    c += OrgChart.server._F(e.assistants[b], a) + a.siblingSeparation / 2
                }
                break;
            case BALKANGraph.orientation.right:
            case BALKANGraph.orientation.right_top:
            case BALKANGraph.orientation.left:
            case BALKANGraph.orientation.left_top:
                var d = e.y + OrgChart.server._F(e, a) + a.siblingSeparation;
                for (var b = 0; b < e.assistants.length; b++) {
                    e.assistants[b].y = d;
                    e.assistants[b].x = e.x;
                    d += OrgChart.server._F(e.assistants[b], a) + a.siblingSeparation / 2
                }
                break
        }
    }
...
OrgChart.server._s = function(n, h, a) {
    ...
    for (var e = BALKANGraph.MAX_DEPTH - h; b != null && c != null && d <= e;) {
        ...
        var t = (c._ar + i + OrgChart.server._S(c, a) + a.subtreeSeparation) - (b._ar + m);
        if (t > 0) {
            var r = n;
            var o = 0;
            for (; r != null && r != g; r = OrgChart.server._C(r)) {
                o++
            }
            if (r != null) {
                var s = n;
                var q = t / o;
                for (; s != g; s = OrgChart.server._C(s)) {
                    s._ar += t;
                    s._0 += t;
                    t -= q
                }
        ...
        d++;
        if (OrgChart.server._Z(b) == 0) {
            b = OrgChart.server._D(n, 0, d)
        } else {
            b = OrgChart.server._W(b)
        }
        if (b != null) {
            c = b.leftNeighbor
        }
...
OrgChart.server._V = function(J, n, D, j, r, s, t, u, G, k, c, d, A, l, C) {
    var H = {};
    var f = {
        top: null,
        left: null,
    ...
    if (G == null) {
        ...
        if (I - j.padding * 2 >= r - t) {
            ...
            K = k.x - I / 2 + OrgChart.server._F(k, j) / 2;
            switch (j.orientation) {
                case BALKANGraph.orientation.right:
                case BALKANGraph.orientation.right_top:
                    K = -((I / 2) - (t - r) / 2);
        ...
        if (m - j.padding * 2 >= s - u) {
            ...
            switch (j.orientation) {
                ...
                case BALKANGraph.orientation.left:
                case BALKANGraph.orientation.right:
                    L = roots[0].y - m / 2 + OrgChart.server._F(k, j) / 2;
                    break
            }
        }
        G = [K, L, I, m]
...
OrgChart.server._aj = function(d, b, c) {
    if (c[b] == null) {
        c[b] = 0
    }
    if (c[b] < d.h) {
    ...
    if (d.assistants.length) {
        for (var a = 0; a < d.assistants.length; a++) {
            OrgChart.server._aj(d.assistants[a], b, c)
        }
    }
};
OrgChart.server._am = function(d, b, c) {
    if (c[b] == null) {
        c[b] = 0
    }
    if (c[b] < d.w) {
    ...
    if (d.assistants.length) {
        for (var a = 0; a < d.assistants.length; a++) {
            OrgChart.server._am(d.assistants[a], b, c)
        }
    }
};
OrgChart.server._ak = function(b, a, c) {
    if (c[a] && c[a].id === b.id) {
        return
    }
    b.leftNeighbor = c[a];
...
OrgChart.server._ah = function(h, c, l, n, e, f, a) {
    if (c <= BALKANGraph.MAX_DEPTH) {
        ...
        if (OrgChart.server._Z(h) != 0) {
            var d = a.levelSeparation;
            if ((a.layout == BALKANGraph.mixed) && h.isLastChild) {
                d = a.mixedHierarchyNodesSeparation
            }
            OrgChart.server._ah(OrgChart.server._W(h), c + 1, l + h._0, n + g + d, e, f, a)
        }
        var j = OrgChart.server._T(h);
        if (j != null) {
            OrgChart.server._ah(j, c, l, n, e, f, a)
        }
    }
};
OrgChart.server._p = function(m, g, h, j, o, a) {
    var f = null;
    m.x = 0;
    m.y = 0;
    m._ar = 0;
    ...
    OrgChart.server._aj(m, g, h);
    OrgChart.server._am(m, g, j);
    OrgChart.server._ak(m, g, o);
    OrgChart.server._an(m, a.orientation);
    if (OrgChart.server._Z(m) == 0 || g == BALKANGraph.MAX_DEPTH) {
        f = OrgChart.server._C(m);
        if (f != null) {
            m._ar = f._ar + OrgChart.server._S(f, a) + a.siblingSeparation
        } else {
            m._ar = 0
        }
    } else {
        var l = OrgChart.server._Z(m);
        for (var c = 0; c < l; c++) {
            var d = OrgChart.server._A(m, c);
            OrgChart.server._p(d, g + 1, h, j, o, a)
        }
        var b = OrgChart.server._W(m);
        var e = OrgChart.server._E(m);
        var k = b._ar + ((e._ar - b._ar) + OrgChart.server._F(e, a)) / 2;
        k -= OrgChart.server._F(m, a) / 2;
        f = OrgChart.server._C(m);
        if (f != null) {
            m._ar = f._ar + OrgChart.server._S(f, a) + a.siblingSeparation;
            m._0 = m._ar - k;
...
OrgChart.server._ai = function(c, a) {
    if (OrgChart.server._Y(c)) {
        return
    }
...
OrgChart.prototype.exportPDF = function(b, a, c) {
    this._o(b, a, c, "pdf", "application/pdf")
};
OrgChart.prototype.exportPNG = function(b, a, c) {
    this._o(b, a, c, "png", "image/png")
};
OrgChart.prototype.exportSVG = function(b, a, c) {
    this._o(b, a, c, "svg", "image/svg+xml")
};
OrgChart.prototype._o = function(c, a, f, b, d) {
    var h = this;
    if (!c) {
        c = "BALKANGraph." + b
    }
...
OrgChart.prototype._i = function(d, b) {
    this.nodeMenuUI.hide();
    this.dragDropMenuUI.hide();
    this.menuUI.hide();
...
OrgChart.prototype._5 = function(d, b) {
    b[0].stopPropagation();
    b[0].preventDefault();
    this.nodeMenuUI.hide();
    this.dragDropMenuUI.hide();
...
OrgChart.prototype._6 = function(b, a) {
    a[0].stopPropagation();
    a[0].preventDefault()
};
BALKANGraph._aZ = function(a) {
    if (!a) {
...
BALKANGraph._aQ = function(a) {
    return a.replace(/^\s+|\s+$/g, "")
};
BALKANGraph._H = function(a) {
...
BALKANGraph._aw = function(e, d, a, b) {
    var c = d.slice(0);
...
OrgChart.prototype._d = function(a) {
    this._z(window, "resize", this._ay)
};
OrgChart.prototype._e = function(m) {
    var m = this.getSvg();
    this._z(m, "mousedown", this._aq);
    if (this.config.mouseScroolBehaviour == BALKANGraph.action.zoom) {
        this._z(m, "DOMMouseScroll", this._aa);
        this._z(m, "mousewheel", this._aa)
    }
    var l = this.getNodeElements();
    for (var d = 0; d < l.length; d++) {
        var k = l[d];
        this._z(k, "mousedown", this._ae);
        this._z(k, "mouseover", this._ac);
        this._z(k, "mouseleave", this._ad);
        this._z(k, "click", this._as)
    }
    var b = this.getExpCollButtons();
    for (var d = 0; d < b.length; d++) {
        var a = b[d];
        this._z(a, "mousedown", this._k);
        this._z(a, "click", this._i)
    }
    var g = this.getMaxMinButtons();
    for (var d = 0; d < g.length; d++) {
        var f = g[d];
        this._z(f, "mousedown", this._6);
        this._z(f, "click", this._5)
    }
    var j = this.getNodeMenuButtons();
    for (var d = 0; d < j.length; d++) {
        var h = j[d];
        this._z(h, "mousedown", this._7);
        this._z(h, "click", this._ax)
    }
    var c = this.getExportMenuButton();
    if (c) {
        this._z(c, "click", this._l)
    }
    var e = this.getLonelyButton();
    if (e) {
        this._z(e, "mousedown", this._4)
    }
};
...
OrgChart.prototype._at = function(a, b) {
...
OrgChart.prototype._aa = function(c, b) {
    var d = this;
...
OrgChart.prototype._ad = function(b, a) {
    if (!this.config.enableDragDrop) {
        return
    }
    this._3 = null;
...
OrgChart.prototype._ae = function(l, d) {
    d[0].stopPropagation();
    d[0].preventDefault();
    if (!this.config.enableDragDrop) {
        return
    ...
    b.style.opacity = 0.7;
    if (a.isChildOfGroup) {
        var j = this.getNodeElement(a._U);
        var e = BALKANGraph._H(j);
        o = o + e[4];
        p = p + e[5];
        g[4] = o;
        g[5] = p;
    ...
    var h = function(q) {
        ...
        if ((a.id == n._3) || (a.isChildOfGroup && a._U == n._3) || (n._3 == null)) {
            m.removeChild(b);
            n._N = null;
            return
        }
        var r = n.getBGNode(n._3);
        if (n.config.dragDropMenu == null || (a.isGroup && !r.isGroup) || a.isAssistant || r.isAssistant) {
            if (n._r(a.id, n._3)) {
                n.linkNode(a.id, n._3)
            } else {
                m.removeChild(b)
            }
        } else {
            if (n._r(a.id, n._3)) {
                n.dragDropMenuUI.show(q.offsetX, q.offsetY, a.id, n._3)
            } else {
                m.removeChild(b)
            }
        }
...
OrgChart.prototype._ay = function(b, a) {
...
OrgChart.prototype._ax = function(d, a) {
    a[0].stopPropagation();
    a[0].preventDefault();
    var b = d.getAttribute("control-node-menu-id");
    this.menuUI.hide();
    var c = BALKANGraph._8(d, a[0], this.getSvg());
    this.nodeMenuUI.show(c.x, c.y, b)
};
OrgChart.prototype._l = function(b, a) {
    a[0].stopPropagation();
    a[0].preventDefault();
    this.nodeMenuUI.hide();
    this.dragDropMenuUI.hide();
...
OrgChart.prototype._4 = function(c, a) {
    a[0].stopPropagation();
    a[0].preventDefault();
...
OrgChart.xScrollUI.prototype.create = function(c) {
    ...
    this.bar.addEventListener("scroll", function() {
        ...
        clearTimeout(this._ap);
        this._ap = setTimeout(function() {
            b.onDrawCallback()
        }, 500)
    })
};
