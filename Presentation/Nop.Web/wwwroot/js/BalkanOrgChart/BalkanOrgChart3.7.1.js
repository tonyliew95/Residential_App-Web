...
var OrgChart = function (b, a) {
    ...
    if (a) {
        for (var e in this.config) {
            if (typeof a[e] === "object" && !Array.isArray(a[e])) {
                this.config[e] = BALKANGraph._aq(this.config[e], a[e])
            } else {
    ...
    if (!BALKANGraph._aX(this.config)) {
        return
    }
    this.server = null;
    this._aE = {};
...
OrgChart.prototype._m = function (s, a, c, i) {
    ...
    if (a === BALKANGraph.action.exporting) {
        ...
        this._K = this.ui.svg(z, m, [(f.minX - this.config.padding), (f.minY - this.config.padding), z, m], this.config, k);
        return
    }
    this.response = t;
...
BALKANGraph = {};
BALKANGraph._w = function (c, d, b, a) {
...
OrgChart.prototype.removeNode = function (b, a) {
    ...
    this._m(false, BALKANGraph.action.update, null, function () {
        BALKANGraph._ax(c.getSvg(), c.getViewBox(), c.response.boundary)
    })
};
...
OrgChart.prototype._aS = function (a, b, f) {
    var d = JSON.parse(JSON.stringify(this.config.tags));
    d[a][b] = f;
...
OrgChart.prototype._ay = function (d, b) {
...
OrgChart.prototype._as = function (b, a, c) {
    c[0] = a;
    c[1] = b;
    this.setViewBox(c);
    this.xScrollUI.setPosition();
...
OrgChart.prototype._az = function (g, b) {
    var h = this;
    ...
    var c = function () {
        if (a.diffX = !0 || a.diffY != 0) {
            BALKANGraph._ax(g, h.getViewBox(), h.response.boundary, function () {
...
OrgChart.editUI.prototype._b = function (h, b) {
    ...
    for (var f = 0; f < d.length; f++) {
        ...
        if (BALKANGraph._3(this.obj.config, d[f])) {
...
OrgChart.editUI.prototype._y = function (l, e) {
    ...
    for (var j = 0; j < g.length; j++) {
        ...
        if (g[j] != "tags") {
            if (BALKANGraph._3(this.obj.config, g[j])) {
                if (o) {
...
OrgChart.editUI.prototype.hide = function (f) {
    ...
    if (f) {
        ...
        var g = this;
        if (this.node.isGroup) {
            this.obj._aS(this.node.id, "groupName", b.Name)
        } else {
...
OrgChart.ui = {
    ...
    defs: function () {
        ...
        for (var c in OrgChart.templates) {
            ...
            if (b.defs) {
                OrgChart.ui._n[c] = BALKANGraph._at();
    ...
    node: function (d, i, c, h, v, z, n, a, e, g) {
        ...
        if (d.isGroup) {
            ...
            if (l) {
                ...
                k = k.replaceAll("{randId}", BALKANGraph._at()).replaceAll("{randId2}", BALKANGraph._at());
                o += k
            }
            o += k
        } else {
            for (var j in n) {
                ...
                if (r != undefined && r != null && s[j] != undefined) {
                    r = BALKANGraph._aD(r, s[j]);
                    var k;
                    if (a === BALKANGraph.action.exporting && BALKANGraph._3(h, j)) {
                        var q = BALKANGraph._at();
                        k = s[j].replace("{val}", q);
                        e(q);
                        BALKANGraph._aZ(r, q, g)
                    } else {
                        k = s[j].replace("{val}", r)
                    }
                    k = k.replaceAll("{randId}", BALKANGraph._at()).replaceAll("{randId2}", BALKANGraph._at());
        ...
        var m = OrgChart.ui._ah(d, s, h, c, n, v, z, a, e, g);
    ...
    _ah: function (c, o, g, b, n, p, q, a, d, e) {
        var k = "";
...
OrgChart.server.prototype._ag = function () {
    ...
    for (var b = 0; b < k.length; b++) {
        ...
        OrgChart.server._au(k[b], 0, 0, 0, d, e, this.config)
    }
    ...
    for (var c in g) {
        var f = g[c];
        OrgChart.server._aQ(f, this.config);
        OrgChart.server._ao(f, a)
    }
    return {
        minX: a.minX,
        minY: a.minY,
...
OrgChart.server.prototype.search = function (n) {
    ...
    for (var e = 0; e < this.config.nodes.length; e++) {
        ...
        for (var k in h) {
            var l = h[k];
            if (l && l.toString().toLowerCase().indexOf(n) != -1 && !BALKANGraph._3(this.config, k)) {
                d.push({
                    id: h.id,
                    propertyName: k,
        ...
        for (var k in d[e].data) {
            ...
            if (k == BALKANGraph.TAGS) {
                ...
                if (k == BALKANGraph.ID) {
                    ...
                    if (BALKANGraph._3(this.config, k)) {
                        continue
                    } else {
                        if (f(k) && !BALKANGraph._3(this.config, k)) {
                            r = r + l + ", "
                        } else {
                            if (!BALKANGraph._3(this.config, k)) {
                                p = p + l + ", "
...
OrgChart.server._h = function (e, a, b, w) {
    ...
    for (var p = 0; p < t.length; p++) {
        ...
        if (v) {
            s.state = v.state;
            OrgChart.server._ap(s, v.groupState, e.orientation)
        } else {
            s.state = BALKANGraph.EXPAND;
            OrgChart.server._ap(s, BALKANGraph.EXPAND, e.orientation)
        }
        OrgChart.server._1(s, u, a, e, b, l);
        OrgChart.server._O(s, u, a, e, b)
    }
...
OrgChart.server._1 = function (j, k, a, e, b, g) {
...
OrgChart.server._O = function (e, f, a, c, b) {
    if (a == BALKANGraph.action.exporting && b.expandChildren == true && e) {
        OrgChart.server._ap(e, BALKANGraph.EXPAND, c.orientation);
        return
    } else {
        if (a == BALKANGraph.action.init && e) {
            if (e.isGroup) {
                var d = c.tags[e.id].groupState;
                OrgChart.server._ap(e, d, c.orientation)
            }
        }
    }
    ...
    OrgChart.server._ap(f[b.id], b.state, c.orientation)
};
OrgChart.server._aj = function (e, f) {
...
OrgChart.server._P = function (j, o, k) {
...
OrgChart.server._ap = function (a, c, b) {
    a.groupState = c;
    OrgChart.server._P(a, a.templateName, b)
};
...
OrgChart.server._4 = function (e, b, f, a) {
...
OrgChart.server._aQ = function (e, a) {
...
OrgChart.server._s = function (n, h, a) {
    ...
    for (var e = BALKANGraph.MAX_DEPTH - h; b != null && c != null && d <= e;) {
        ...
        var t = (c._av + i + OrgChart.server._X(c, a) + a.subtreeSeparation) - (b._av + m);
        if (t > 0) {
            var r = n;
            var o = 0;
...
OrgChart.server._ai = function (d, b, c) {
    if (c[b] == null) {
        c[b] = 0
    }
    if (c[b] < d.h) {
    ...
    if (d.assistants.length) {
        for (var a = 0; a < d.assistants.length; a++) {
            OrgChart.server._ai(d.assistants[a], b, c)
        }
    }
};
...
OrgChart.server._al = function (b, a, c) {
    if (c[a] && c[a].id === b.id) {
        return
    }
    b.leftNeighbor = c[a];
...
OrgChart.server._au = function (h, c, l, n, e, f, a) {
    if (c <= BALKANGraph.MAX_DEPTH) {
        ...
        if (OrgChart.server._W(h) != 0) {
            ...
            OrgChart.server._au(OrgChart.server._S(h), c + 1, l + h._aa, n + g + d, e, f, a)
        }
        var j = OrgChart.server._G(h);
        if (j != null) {
            OrgChart.server._au(j, c, l, n, e, f, a)
        }
    }
};
OrgChart.server._Q = function (m, g, h, j, o, a) {
    ...
    OrgChart.server._ai(m, g, h);
    OrgChart.server._ak(m, g, j);
    OrgChart.server._al(m, g, o);
    OrgChart.server._aj(m, a.orientation);
    if (OrgChart.server._W(m) == 0 || g == BALKANGraph.MAX_DEPTH) {
        f = OrgChart.server._R(m);
        if (f != null) {
            m._av = f._av + OrgChart.server._X(f, a) + a.siblingSeparation
        } else {
            m._av = 0
        }
    } else {
        ...
        var k = b._av + ((e._av - b._av) + OrgChart.server._V(e, a)) / 2;
...
OrgChart.server._ao = function (c, a) {
...
OrgChart.prototype._7 = function (d, b) {
    b[0].stopPropagation();
    b[0].preventDefault();
    this.nodeMenuUI.hide();
    this.dragDropMenuUI.hide();
...
BALKANGraph._ax = function (e, d, a, b) {
    var c = d.slice(0);
...
BALKANGraph._aZ = function (d, c, a) {
...
OrgChart.prototype._d = function (a) {
    this._z(window, "resize", this._an)
};
OrgChart.prototype._e = function (m) {
    var m = this.getSvg();
    this._z(m, "mousedown", this._az);
    if (this.config.mouseScroolBehaviour == BALKANGraph.action.zoom) {
        this._z(m, "DOMMouseScroll", this._aw);
        this._z(m, "mousewheel", this._aw)
    }
    var l = this.getNodeElements();
    for (var d = 0; d < l.length; d++) {
        var k = l[d];
        this._z(k, "mousedown", this._ac);
        this._z(k, "mouseover", this._af);
        this._z(k, "mouseleave", this._ar);
        this._z(k, "click", this._ae)
    }
    ...
    for (var d = 0; d < g.length; d++) {
        var f = g[d];
        this._z(f, "mousedown", this._8);
        this._z(f, "click", this._7)
    }
    var j = this.getNodeMenuButtons();
    for (var d = 0; d < j.length; d++) {
        var h = j[d];
        this._z(h, "mousedown", this._9);
        this._z(h, "click", this._ad)
    }
...
OrgChart.prototype._aw = function (c, b) {
    var d = this;
...
OrgChart.prototype._af = function (d, b) {
...
OrgChart.prototype._ar = function (b, a) {
    if (!this.config.enableDragDrop) {
        return
    }
    this._5 = null;
...
OrgChart.prototype._ac = function (l, d) {
    d[0].stopPropagation();
    d[0].preventDefault();
    if (!this.config.enableDragDrop) {
        return
    ...
    var h = function (q) {
        ...
        if ((a.id == n._5) || (a.isChildOfGroup && a._J == n._5) || (n._5 == null)) {
            m.removeChild(b);
            n._U = null;
            return
        }
        var r = n.getBGNode(n._5);
        if (n.config.dragDropMenu == null || (a.isGroup && !r.isGroup) || a.isAssistant || r.isAssistant) {
            if (n._r(a.id, n._5)) {
                n.linkNode(a.id, n._5)
            } else {
                m.removeChild(b)
            }
        } else {
            if (n._r(a.id, n._5)) {
                n.dragDropMenuUI.show(q.offsetX, q.offsetY, a.id, n._5)
            } else {
                m.removeChild(b)
            }
        }
...
OrgChart.prototype._ae = function (d, b) {
    this.searchUI.hide();
    this.nodeMenuUI.hide();
    this.dragDropMenuUI.hide();
    this.menuUI.hide();
...
OrgChart.prototype._ad = function (d, a) {
    a[0].stopPropagation();
    a[0].preventDefault();
    var b = d.getAttribute("control-node-menu-id");
    this.menuUI.hide();
    var c = BALKANGraph._0(d, a[0], this.getSvg());
...
OrgChart.prototype._6 = function (c, a) {
    a[0].stopPropagation();
    a[0].preventDefault();
...
OrgChart.xScrollUI.prototype.create = function (c) {
    ...
    this.bar.addEventListener("scroll", function () {
        ...
        clearTimeout(this._aA);
        this._aA = setTimeout(function () {
            b.onDrawCallback()
        }, 500)
    })
};
