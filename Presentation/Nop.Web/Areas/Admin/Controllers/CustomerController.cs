...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        private readonly IMspHelper _mspHelper; //Jerry 20180814 MSP-45
        ...
        public CustomerController(ICustomerService customerService,
            ...
            , IMspHelper mspHelper //Jerry 20180814 MSP-45
            )
        {
            ...
            this._mspHelper = mspHelper; //Jerry 20180814 MSP-45
        }
        ...
        //protected virtual List<CustomerModel.SecurityQuestionAndAnswerModel> GetSecurityQuestionAndAnswerRecords(Customer customer) //Jerry 20180814 MSP-45
        //{
        //    if (customer == null)
        //        throw new ArgumentNullException(nameof(customer));

        //    var result = new List<CustomerModel.SecurityQuestionAndAnswerModel>();
        //    var securityQuestion = _customerService.GetSecurityQuestionAndAnswer(customer.Id);
        //    int loop = 0;
        //    foreach (var record in securityQuestion)
        //    {
        //        loop++;
        //        var question = record.GetType().GetProperty("Question").GetValue(record, null).ToString();
        //        var answer = record.GetType().GetProperty("Answer").GetValue(record, null).ToString();

        //        result.Add(new CustomerModel.SecurityQuestionAndAnswerModel
        //        {
        //            SecurityQuestionLabel = _localizationService.GetResource("Admin.Customers.Customers.Security.Question").Trim() + " " + loop,
        //            SecurityQuestion = question,
        //            SecurityAnswerLabel = _localizationService.GetResource("Admin.Customers.Customers.Security.Answer").Trim() + " " + loop,
        //            //SecurityAnswer = new string('*', answer.Length), //Jerry 20180824 MSP-45
        //            SecurityAnswer = (string.IsNullOrWhiteSpace(answer) ? "" : "******"), //Jerry 20180824 MSP-45
        //        });
        //    }
        ...
        protected virtual void PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties)
        {
            ...
            if (customer != null)
            {
                ...
                if (!excludeProperties)
                {
                    ...
                    var password = _customerService.GetCustomerPasswords(customer.Id);
                    model.Password = (password.Count <= 0 ? "" : "******");

                    //var transactionPassword = _customerService.GetCustomerTransactionPasswords(customer.Id);
                    //model.TransactionPassword = (transactionPassword.Count <= 0 ? "" : "******");
                    //Jerry 20180824 MSP-45 /\
            ...
            //PrepareMembershipModel(model, customer); //Jerry 20180814 MSP-45
        }
        ...
        protected virtual void PrepareMembershipModel(CustomerModel model, Customer customer) //Jerry 20180814 MSP-45
        {
            if (customer != null)
            {
                #region if (customer != null)

                var memberTree = _customerService.GetMemberTree(customer.Id);
                if (memberTree != null)
                {
                    model.ReferralId = memberTree.ParentID; //Jerry 20180824 MSP-45
                    model.ReferralName = memberTree.IntroducerGlobalGUID; //Jerry 20181008 MSP-249
                    model.SSO_GUID = memberTree.GlobalGUID; //Jerry 20181008 MSP-251
                    model.Deposit_BTC_Address = memberTree.DepositWalletAddress;
                    model.Withdrawal_BTC_Address = memberTree.WithdrawalWalletAddress;
                    model.IPAddress_Finsys = memberTree.IPAddress_Finsys;
                    model.IPAddress_Game = memberTree.IPAddress_Game;
                }

                //Jerry 20180824 MSP-45 \/
                if (model.ReferralId > 0)
                {
                    var referral = _customerService.GetCustomerById(model.ReferralId);
                    if (referral != null)
                    {
                        //Jerry 20181008 MSP-249 \/
                        //model.ReferralName = referral.GetFullName();
                        model.ReferralName = referral.Username;
                        //Jerry 20181008 MSP-249 /\
                    }
                }
                //Jerry 20180824 MSP-45 /\

                //model.SecurityQuestionAndAnswer = GetSecurityQuestionAndAnswerRecords(customer); //Atiqah 20190130 MDT-205
                model.Deposit_Returned_Amount = _customerService.GetDepositReturnedAmountAndPercentage(customer.Id);
                model.Deposit_Not_Returned_Amount = _customerService.GetDepositNotReturnedAmountAndPercentage(customer.Id);
                //Jerry 20181022 MSP-376 \/
                #region Comment Code
                ////Jerry 20181008 MSP-252 \/
                //model.Total_Self_Reward_Earned = _customerService.GetTotalSelfConsumptionRewardEarned(customer.Id);
                //model.Total_Team_Reward_Earned = _customerService.GetTotalTeamConsumptionRewardEarned(customer.Id);
                ////Jerry 20181008 MSP-252 /\
                #endregion Comment Code
                //Jerry 20181022 MSP-376 /\

                var wallet = _customerService.GetMemberWallet(customer.Id);
                if (wallet != null)
                {
                    model.Available_Balance = _mspHelper.TruncateDecimalToString_MBTC(wallet.Mbtc);
                    //model.Redeem_Wallet = _mspHelper.TruncateDecimalToString_MBTC(wallet.Mbtc_Float); //Jerry 20180824 MSP-45
                    model.Redeem_Wallet = _mspHelper.TruncateDecimalToString_MBTC(wallet.Profit_DP_Float); //Tony Liew 20181029 MSP-398
                    model.Deposit_Wallet = _mspHelper.TruncateDecimalToString_MBTC(wallet.Deposit_Total);
                    //model.Total_Reward_Earned = _mspHelper.TruncateDecimalToString_MBTC(wallet.Deposit_Return_Total + wallet.Profit_Total); //LeeChurn 20181102 MSP-441
                    model.Total_Reward_Earned = _mspHelper.TruncateDecimalToString_MBTC(wallet.Profit_Total);                                 //LeeChurn 20181102 MSP-441
                    //Jerry 20180824 MSP-45 \/
                    //model.Current_Score_Y = _mspHelper.TruncateDecimalToString_MBTC(wallet.Score_Y);
                    //model.Current_Score_Y_Percentage = Math.Floor(wallet.ScorePct_Y * 100).ToString();
                    //model.Current_Score_Z = _mspHelper.TruncateDecimalToString_MBTC(wallet.Score_Z);
                    //model.Current_Score_Z_Percentage = Math.Floor(wallet.ScorePct_Z * 100).ToString();
                    model.Current_Score_Y = _mspHelper.TruncateDecimalToString_MBTC(wallet.Score_Y) + " (" + Math.Floor(wallet.ScorePct_Y * 100).ToString() + "%)";
                    model.Current_Score_Z = _mspHelper.TruncateDecimalToString_MBTC(wallet.Score_Z) + " (" + Math.Floor(wallet.ScorePct_Z * 100).ToString() + "%)";
                    //Jerry 20180824 MSP-45 /\
                    //Jerry 20181022 MSP-376 \/
                    model.Total_Self_Reward_Earned = _mspHelper.TruncateDecimalToString_MBTC(wallet.Profit_CP_Self);
                    model.Total_Team_Reward_Earned = _mspHelper.TruncateDecimalToString_MBTC(wallet.Profit_CP);
                    //Jerry 20181022 MSP-376 /\
                }

                //Jerry 20181008 MSP-251 \/
                #region Comment Code
                ////Jerry 20180912 MSP-45 \/
                //var memberInterface = _customerService.GetConsumerInterface(customer.Id);
                //if (memberInterface != null)
                //{
                //    model.SSO_GUID = memberInterface.GlobalUserID;
                //}
                ////Jerry 20180912 MSP-45 /\
                #endregion Comment Code
                //Jerry 20181008 MSP-251 /\

                #endregion if (customer != null)
            }
        }
        ...
        public virtual IActionResult List()
        {
            ...
            var model = new CustomerListModel
            {
                ...
                IsCS = _workContext.CurrentCustomer.IsCS(), // Tony Liew 20181030 MSP-411 
                IsCSAdmin = _workContext.CurrentCustomer.IsCSAdmin(), // Tony Liew 20181030 MSP-411 
                IsAdmin = _workContext.CurrentCustomer.IsAdmin(), // Tony Liew 20181030 MSP-411
                IsAOD = _workContext.CurrentCustomer.IsAOD(),
                IsFinance = _workContext.CurrentCustomer.IsFinance()
            };
            var allRoles = _customerService.GetAllCustomerRoles(true);
            foreach (var role in allRoles)
            {
        ...
        public virtual IActionResult CustomerList(DataSourceRequest command, CustomerListModel model, int[] searchCustomerRoleIds)
        {
            ...
            var customers = _customerService.GetAllCustomers(_workContext.CurrentCustomer.IsCS(), // Tony Liew 20181030 MSP-411 
                _workContext.CurrentCustomer.IsCSAdmin(), // Tony Liew 20181030 MSP-411 
                _workContext.CurrentCustomer.IsAdmin(),// Tony Liew 20181030 MSP-411 
                _workContext.CurrentCustomer.IsAOD(),
                _workContext.CurrentCustomer.IsFinance(),
        ...
        public virtual IActionResult Create()
        {
            ...
            {
                IsCSAdmin = _workContext.CurrentCustomer.IsCSAdmin(),
                IsAdmin = _workContext.CurrentCustomer.IsAdmin(),
                IsCS = _workContext.CurrentCustomer.IsCS(),
                IsAOD = _workContext.CurrentCustomer.IsAOD(),
                IsFinance = _workContext.CurrentCustomer.IsFinance(),
                Active = true,
            };
            ...
            if (model.IsCS)
                model.IsAbleToCreate = false;
            else if (model.IsCSAdmin)
            {
                model.IsAbleToCreate = true;
            ...
            model.AvailableCustomerRoles = CSAdminRoleFeature(model.AvailableCustomerRoles, _workContext.CurrentCustomer);  //LeeChurn 20181101 MSP-410
            return View(model);
        }
        ...
        public virtual IActionResult Create(CustomerModel model, bool continueEditing)
        {
            ...
            if (newCustomerRoles.Any() && newCustomerRoles.FirstOrDefault(c => c.SystemName == SystemCustomerRoleNames.Registered) != null && !Core.CommonHelper.IsValidEmail(model.Email))
            {
                ModelState.AddModelError("", _localizationService.GetResource("Admin.Customers.Customers.ValidEmailRequiredRegisteredRole"));
                base.ErrorNotification(_localizationService.GetResource("Admin.Customers.Customers.ValidEmailRequiredRegisteredRole"), false);
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                customer = _customerService.PrepareCustomerSyncUpdatedInfo(customer, null, GlobalSettingEnum.MSPPrepareCustomerSyncTypeFirstTimeCreate); //LeeChurn 20181031 MSP-410
                _customerService.InsertCustomer(customer);

                //password
                if (!string.IsNullOrWhiteSpace(model.Password))
                {
                    var changePassRequest = new CustomerRegistrationRequest(customer, model.Email, model.Email, model.Password, _customerSettings.DefaultPasswordFormat, 1);
                    var changePassResult = _customerRegistrationService.RegisterCustomer(changePassRequest);
                ...
                customer = _customerService.PrepareCustomerSyncUpdatedInfo(customer, _workContext.CurrentCustomer, GlobalSettingEnum.MSPPrepareCustomerSyncTypeBackOffice); //LeeChurn 20181102 MSP-439 //LeeChurn 20181031 MSP-410
                ...
                if (customer.IsVendor() && customer.VendorId == 0)
                {
                    ...
                    customer = _customerService.PrepareCustomerSyncUpdatedInfo(customer, _workContext.CurrentCustomer, GlobalSettingEnum.MSPPrepareCustomerSyncTypeBackOffice); //LeeChurn 20181102 MSP-439 //LeeChurn 20181031 MSP-409
                    _customerService.UpdateCustomer(customer);
        ...
        public virtual IActionResult Edit(int id)
        {
            ...
            //var membershipCustomer = _customerService.GetMemberTree(id);
            if (customer == null || customer.Deleted)
                //No customer found with the specified id
                return RedirectToAction("List");
            //Tony 20181101 MSP-411 \/ 
            var consumer = Customer_Type.Consumer.ToDescription<Customer_Type>();
            var merchant = Customer_Type.Merchant.ToDescription<Customer_Type>();
            ...
            model.CustomerIsMember = customer.IsMember;
            //if (membershipCustomer != null)
            //{
            //    membershipCustomer.UserRole = membershipCustomer.UserRole.ToUpper();
            //    model.UserRole = membershipCustomer.UserRole == Customer_Type.Consumer.ToValue<Customer_Type>() ? consumer : merchant;
            //    model.WithdrawalLimit = membershipCustomer.WithdrawalLimit;//Tony Liew 20181121 MSP-521 
            //    model.WithdrawalLimitVIP = membershipCustomer.WithdrawalLimitVIP;//Tony Liew 20181121 MSP-521 
            //    model.WithdrawalEnabled = membershipCustomer.IsWithdrawalEnabled;//Tony Liew 20181121 MSP-521 
            //}

            //model.ChangedPasswordEnabled = ChangePasswordEnabled(model); //LeeChurn 20181101 MSP-410
            if (model.IsCS)
            {
                if (id == _workContext.CurrentCustomer.Id)
                    model.ChangedPasswordEnabled = true;
            }
            //wailiang 20181213 MDT-133 \/
            if (model.IsFinance)
            {
            ...
            else if (model.IsCSAdmin)
            {
                ...
                else if (id != _workContext.CurrentCustomer.Id)
                {
                    if (customer.IsCS())
                    {
                        model.ChangedPasswordEnabled = true;
            ...
            else if (model.IsAdmin)
            {
                ...
                {
                    if (customer.IsCS() || customer.IsCSAdmin() || customer.IsAdmin() || customer.IsAOD() || customer.IsFinance()) //wailiang 20181214 MDT-133
                    {
                        model.ChangedPasswordEnabled = true;
                        model.IsEditable = true;
                        model.IsAbleToCreate = true;
            ...
            model.IsStatusEditable = IsStatusEditable(customer); /*Lee Churn 20181102 MSP-439  */
        ...
        public virtual IActionResult Edit(CustomerModel model, bool continueEditing)
        {
            ...
            model.IsStatusEditable = IsStatusEditable(customer);                                   /*Lee Churn 20181102 MSP-439  */
            var prevCustomerRole = customer.GetCustomerRoleIds().OfType<int>().ToList(); //LeeChurn 20181102 MSP-439
            //LeeChurn 20181102 MSP-450 \/
            //if (customer.IsMember)
            //{
            //    model.SelectedCustomerRoleIds = customer.GetCustomerRoleIds().OfType<int>().ToList(); //LeeChurn 20181102 MSP-439
            //    //Tony Liew 20181121 MDT-8  \/
            //    var membershipCustomer = _customerService.GetMemberTree(model.Id);
            //    //Chew 20181212 MSP-546  \/
            //    if (membershipCustomer != null)
            //    {
            //        membershipCustomer.WithdrawalLimit = model.WithdrawalLimit;
            //        membershipCustomer.WithdrawalLimitVIP = model.WithdrawalLimitVIP;
            //        membershipCustomer.IsWithdrawalEnabled = model.WithdrawalEnabled;
            //        membershipCustomer.UpdatedOnUtc = DateTime.UtcNow;
            //        _customerService.UpdateMemberWithdrawalLimit(membershipCustomer);
            //    }
            //    //Chew 20181212 MSP-546  /\
            //    //Tony Liew 20181121 MDT-8  /\
            //}
            //else
            //{
            //    if (customer.IsCSAdmin() || customer.IsCS() || customer.IsAOD() || customer.IsFinance()) //wailiang 20181213 MDT-133
            //    {
            //        //do nothing
            //    }
            //}
        ...
        public virtual IActionResult ChangePassword(CustomerModel model)
        {
            ...
            #region Tony 20181030 MSP-411 
            //ensure that the current customer cannot change passwords of "Administrators" , "CS Admin" and "CS" if he's not an CS himself
            if (customer.IsCS() && _workContext.CurrentCustomer.IsCS() && customer.IsCSAdmin() && customer.IsAdmin())
            {
                ErrorNotification(_localizationService.GetResource("Admin.Customers.Customers.OnlyAdminCanChangePassword"));
                return RedirectToAction("Edit", new { id = customer.Id });
            }

            //ensure that the current customer cannot change passwords of "Administrators" and "CS Admin" if he's not an CS admin himself
            else if (_workContext.CurrentCustomer.IsCSAdmin() && customer.IsCSAdmin() && customer.IsAdmin())
            {
                ErrorNotification(_localizationService.GetResource("Admin.Customers.Customers.OnlyAdminCanChangePassword"));
                return RedirectToAction("Edit", new { id = customer.Id });
            }
            ...
            else if (customer.IsAdmin() && !_workContext.CurrentCustomer.IsAdmin())
            {
                ErrorNotification(_localizationService.GetResource("Admin.Customers.Customers.OnlyAdminCanChangePassword"));
                return RedirectToAction("Edit", new { id = customer.Id });
            }
            ...
            // LeeChurn 20181101 MSP-410 \/
            customer = _customerService.PrepareCustomerSyncUpdatedInfo(customer, _workContext.CurrentCustomer, Nop.Services.Enumeration.GlobalSettingEnum.MSPPrepareCustomerSyncTypeBackOffice);
            _customerService.UpdateCustomer(customer);
            // LeeChurn 20181101 MSP-410 /\
            return RedirectToAction("Edit", new { id = customer.Id });
        }
        ...
        [HttpPost, ActionName("Edit")]
        [FormValueRequired("changetransactionpassword")]
        public virtual IActionResult ChangeTransactionPassword(CustomerModel model) //Jerry 20180814 MSP-45
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            //ensure that the current customer cannot change transaction passwords of "Administrators" if he's not an admin himself
            if (customer.IsAdmin() && !_workContext.CurrentCustomer.IsAdmin())
            {
                ErrorNotification(_localizationService.GetResource("Admin.Customers.Customers.OnlyAdminCanChangeTransactionPassword"));
                return RedirectToAction("Edit", new { id = customer.Id });
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                var changePassResult = _customerRegistrationService.ChangeTransactionPassword(changePassRequest);
                if (changePassResult.Success)
                    SuccessNotification(_localizationService.GetResource("Admin.Customers.Customers.TransactionPasswordChanged"));
                else
                    foreach (var error in changePassResult.Errors)
                        ErrorNotification(error);
            }
        ...
        public virtual IActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                ...
                if (!Core.CommonHelper.IsValidEmail(customer.Email))
        ...
        #region Only allow CS Role created for CS Administrator user        
        private List<SelectListItem> CSAdminRoleFeature(List<SelectListItem> customerRole, Customer customer) //LeeChurn 20181030 MSP-410
        {
            bool CSAdminRoleFeature = true;
            if (CSAdminRoleFeature)
            {

                if (customer.IsCSAdmin())
                {

                    var allRoles = _customerService.GetAllCustomerRoles(true);
                    string registerRoleName = allRoles.FirstOrDefault(c => c.SystemName == SystemCustomerRoleNames.Registered) != null ? allRoles.FirstOrDefault(c => c.SystemName == SystemCustomerRoleNames.Registered).Name : string.Empty;
                    string customerServiceRoleName = allRoles.FirstOrDefault(c => c.SystemName == SystemCustomerRoleNames.CS) != null ? allRoles.FirstOrDefault(c => c.SystemName == SystemCustomerRoleNames.CS).Name : string.Empty;

                    string[] roleAllowedforCSAdmin = { registerRoleName, customerServiceRoleName };
                    var newcustomerRole = from role in customerRole
                                          where roleAllowedforCSAdmin.Contains(role.Text)
                                          select role;

                    return newcustomerRole.ToList<SelectListItem>();
        ...
        #region Only allow CS User / CS Administrator status to be changed
        private bool IsStatusEditable(Customer customer)
        {
            bool result = false;

            var RoleIds = customer.GetCustomerRoleIds();
            int[] UserRoleAllowEditStatus = { _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.CS).Id,
                                              _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.CSAdministrator).Id,
                                                _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.Administrators).Id,
                                                _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.AOD).Id,
                                                _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.Finance).Id,
            };
            if (UserRoleAllowEditStatus.Intersect(RoleIds).Any())
            {
                result = true;
            }
            return result;
