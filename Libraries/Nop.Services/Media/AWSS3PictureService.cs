...
namespace Nop.Services.Media
{
    ...
    /// Picture service
    /// </summary>
    public partial class AWSS3PictureService : IPictureService
    {
        #region Const
        ...
        public MediaSettings MediaSettings { get { return _mediaSettings; } }
        #endregion
        ...
        public AWSS3PictureService(IRepository<Picture> pictureRepository,
        ...
        protected virtual string GetThumbLocalPath_Pnc(string thumbFileName) //JK 20180507 GLPEC-1221
        {
            var thumbsDirectoryPath = CommonHelper.MapPath("~/content/images/thumbs");
            if (_mediaSettings.MultipleThumbDirectories)
            {
        ...
        protected virtual string GetThumbUrl_Pnc(string thumbFileName, string storeLocation = null) //JK 20180507 GLPEC-1221
        {
            storeLocation = !String.IsNullOrEmpty(storeLocation)
                                    ? storeLocation
                                    : _webHelper.GetStoreLocation();
        ...
        protected virtual bool GeneratedThumbExists_Pnc(string thumbFilePath, string thumbFileName)
        {
            return File.Exists(thumbFilePath);
        }

        /// <summary>
        /// Save image of binary type into s3 server
        /// </summary>
        /// <param name="thumbFilePath"></param>
        /// <param name="thumbFileName"></param>
        /// <param name="binary"></param>
        public static void SaveThumbTos3(string folderName, string thumbFileName, byte[] binary)
        {
            string accessKey = Config.AWSS3AccessKey;
            string secretKey = Config.AWSS3SecretKey;

            S3Service service = new S3Service(accessKey, secretKey, Config.PictureS3Bucket, Config.Environment + folderName + "/" + thumbFileName);

            Stream stream = new MemoryStream(binary);
            service.UploadFile(stream);
        }

        public static string GetPaymentProofUrlFroms3(string username, string thumbFileName)
        {
            return GetThumbUrlFroms3(username + "/payment", thumbFileName);
        }

        public static string GetThumbUrlFroms3(string folderName, string thumbFileName)
        {
            return  "/" + folderName + "/" + thumbFileName;
        }
        ...
        public virtual string GetOriginalPictureUrl(int pictureId)
        {
            return GetPictureUrl(pictureId);
        }

        public virtual string GetOriginalPictureUrl(Picture picture)
        {
            if (picture == null)
            {
            ...
            return GetPictureUrl(picture.Id);
        }

        public virtual string GetThumbnailPictureUrl(int pictureId, int maxImageWidth)
        {
            return GetPictureUrl(pictureId, maxImageWidth);
        }

        public virtual string GetThumbnailPictureUrl(Picture picture, int maxImageWidth)
        {
            if (picture == null)
            {
                return GetDefaultPictureUrl();
            ...
            return GetPictureUrl(picture.Id, maxImageWidth);
        }
    }
}
